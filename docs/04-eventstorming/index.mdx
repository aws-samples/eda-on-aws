...
support would be examples of sub-domains. Within each of these sub-domains would be boundaries where
engineering teams would build solutions. The customer support sub-domain could operate multiple
systems to handle returns, billing, website orders, etc.
...
the same: come to a shared understand of a business process and model so that you can build and
...
<h2>
  EventStorming is a workshop-based approach used to break down a non-trivial problem domain with
...
- **Process**: This is the next level down in terms of details. Once a sub-domain or business
